...
/*
 ** Initialize webfm on the current page.
 */
if (Drupal.jsEnabled) {
...
Webfm.jsUpload.prototype.oncomplete = function (data) {
	...
	if (this.confirm) {
		...
		listener = Webfm.eventListenerAdd(Webfm.eventListeners, replaceButton, "click", function (e) {
																							cp.submit();
																							Webfm.stopEvent(e);
																						});
...
Webfm.list = function (parent, id, type, class_name, dd_enable, dir_menu, file_menu) {
	...
	listener = Webfm.eventListenerAdd(Webfm.eventListeners, elA, "click", function (e) { 
																			wl.refresh(Webfm.current);
																			Webfm.stopEvent(e);
																		});
...
Webfm.list.prototype.bcrumb_listenerfn_click = function (cp) {
...
Webfm.list.prototype.bcrumb = function () {
	var cp, i, pth, elA, listener, elSpan;
	cp = this;
	pth = [];
	...
	for (i = 0; i < this.content.bcrumb.length - 1; i = i + 1) {
		...
		//IE fix
		listener = Webfm.eventListenerAdd(Webfm.eventListeners, elA, "click", Webfm.list.prototype.bcrumb_listenerfn_click(cp));
...
Webfm.list.prototype.callback = function (string, xmlhttp, cp) {
	...
	if (xmlhttp.status == 200) {
		...
		if (cp.content.status) {
			...
			if (cp.rename_dir) {
				found = false;
				for (i = 0; i < Webfm.dirListObj.dirrows.length; i = i + 1) {
...
Webfm.list.prototype.adminCtl = function (admin) {
	...
	if (admin) {
		if (Webfm.$('webfm-bcrumb-td').colSpan == col_span) {
			...
			listener = Webfm.eventListenerAdd(Webfm.eventListeners, elA, "mouseover", function (e) {
																						wl.hover(e, true, 0);
																					});
			listener = Webfm.eventListenerAdd(Webfm.eventListeners, elA, "mouseout", function (e) {
																						wl.hover(e, false, 0);
																					});
			elSpan.appendChild(elA);
...
Webfm.list.prototype.loadList = function (sortcol) {
	...
	if (this.content.dirs.length) {
		...
		for (i = 0; i < this.content.dirs.length; i = i + 1) {
	...
	if (this.content.files.length) {
		...
		for (i = 0; i < this.content.files.length; i = i + 1) {
...
Webfm.dirrow = function (parent, dir, index, dd_enable, menu, eventListenerArr) {
	...
	if (dd_enable) {
		listener = Webfm.eventListenerAdd(eventListenerArr, this.element, "mousedown", function (e) {
																						Webfm.contextMenuObj.hideContextMenu(e);
																						dr.select(e);
																					});
	}
	else {
		listener = Webfm.eventListenerAdd(eventListenerArr, this.element, "click", function (e) {
																					Webfm.contextMenuObj.hideContextMenu(e);
																					if (Webfm.renameActive === false) { 
																						Webfm.selectDir(dr.element.title);
																					} 
																				});
	}
	if (this.draggable) {
		listener = Webfm.eventListenerAdd(eventListenerArr, this.element, "mouseover", function () {
																						dr.hover(dr.element, true);
																					});
		listener = Webfm.eventListenerAdd(eventListenerArr, this.element, "mouseout", function () { 
																						dr.hover(dr.element, false); 
																					});
	}
	if (Webfm.browser.isOP) {
		listener = Webfm.eventListenerAdd(eventListenerArr, this.element, "mousedown", function (e) { 
																						if (e && e.button === 0 && e.altKey === true) { 
																							if (Webfm.renameActive === false) {
																								Webfm.contextMenuObj.showContextMenu(e, dr);
																							}
																							Webfm.stopEvent(e);
																						}
																					});
		...
		listener = Webfm.eventListenerAdd(eventListenerArr, this.element, "contextmenu", function (e) { 
																							if (Webfm.renameActive === false) {
																								Webfm.contextMenuObj.showContextMenu(e, dr);
																							}
																							Webfm.stopEvent(e); 
...
Webfm.filerow = function (parent, fileObj, idtype, index, dd_enable, file_menu, eventListenerArr) {
	...
	if (dd_enable) {
		...
		listener = Webfm.eventListenerAdd(eventListenerArr, this.element, "click", function (e) {
																					Webfm.contextMenuObj.hideContextMenu(e);
																					if (Webfm.renameActive === false) { 
																						Webfm.selectFile(fr.clickObj.title, fr.element);
																					} 
																				});
	}
	if (this.draggable) {
		listener = Webfm.eventListenerAdd(eventListenerArr, this.element, "mouseover", function () {
																										fr.hover(fr.element, true);
																									});
		listener = Webfm.eventListenerAdd(eventListenerArr, this.element, "mouseout", function () {
																										fr.hover(fr.element, false);
																									});
	}
	if (Webfm.browser.isOP) {
		listener = Webfm.eventListenerAdd(eventListenerArr, this.element, "mouseup", function (e) {
																						if (e && e.button === 0 && e.altKey === true) {
																							if (Webfm.renameActive === false) {
																								Webfm.contextMenuObj.showContextMenu(e, fr);
																								Webfm.stopEvent(e);
																							}
																						}
																					});
...
Webfm.tree.prototype.buildTreeRecur_listenerfn_mouseover = function (cp) {
	 var listenerfn = function (e) { 
		 					cp.hover(e, true);
						};
...
Webfm.tree.prototype.buildTreeRecur_listenerfn_mouseout = function (cp) {
	 var listenerfn = function (e) { 
		 					cp.hover(e, false);
						};
	...
	if (Webfm.isArray(content)) {
		...
		if (sortDir.length) {
			...
			for (j = 0; j < sortDir.length; j = j + 1) {
				...
				listener = Webfm.eventListenerAdd(this.eventListeners, elDiv, "mouseover", Webfm.tree.prototype.buildTreeRecur_listenerfn_mouseover(cp));
				listener = Webfm.eventListenerAdd(this.eventListeners, elDiv, "mouseout", Webfm.tree.prototype.buildTreeRecur_listenerfn_mouseout(cp));
...
Webfm.tree.prototype.init = function () {
	...
	if (dirItems.length) {
		for (i = 0; i < dirItems.length; i = i + 1) {
...
Webfm.treeNode = function (parent, path, id, expImg, menu, eventListenerArr) {
	...
	this.menu = menu;
	if (typeof this.dd != "undefined") {
		listener = Webfm.eventListenerAdd(eventListenerArr, parent, "mousedown", function (e) {
																					Webfm.contextMenuObj.hideContextMenu(e);
																					tn.select(e);
																				});
	}
	else {
		listener = Webfm.eventListenerAdd(eventListenerArr, parent, "click", function (e) { 
																				Webfm.selectDir(tn.clickObj.title); 
																				Webfm.stopEvent(e); 
																			});
	}
	if (Webfm.browser.isOP) {
		listener = Webfm.eventListenerAdd(eventListenerArr, parent, "mouseup", function (e) {
																				if (e && e.button === 0 && e.altKey === true) { 
																					if (Webfm.renameActive === false) { 
																						Webfm.contextMenuObj.showContextMenu(e, tn); 
																					} 
																					Webfm.stopEvent(e); 
																				}
																			});
		...
		listener = Webfm.eventListenerAdd(eventListenerArr, parent, "contextmenu", function (e) {
																					if (Webfm.renameActive === false) {
																						Webfm.contextMenuObj.showContextMenu(e, tn);
																					}
																					Webfm.stopEvent(e);
...
Webfm.context.prototype.showContextMenu_listenerfn_mousedown = function (cp, obj) {
	var listenerfn = function (e) { 
							cp.selectMenu(e, obj);
							cp.hideContextMenu(e);
							Webfm.stopEvent(e);
						};
	return listenerfn;
};

Webfm.context.prototype.showContextMenu_listenerfn_mouseover = function (cp) {
	var listenerfn = function (e) {
							cp.hover(e, true);
						};
	return listenerfn;
...
Webfm.context.prototype.showContextMenu_listenerfn_mouseout = function (cp) {
	var listenerfn = function (e) {
							cp.hover(e, false);
						};
	return listenerfn;
...
Webfm.context.prototype.showContextMenu = function (event, obj) {
	...
	if (obj.menu !== null || typeof obj.menu !== 'undefined' || obj.menu !== false) {
		...
		for (j = 0; j < obj.menu.length; j = j + 1) {
			...
			listener = Webfm.eventListenerAdd(Webfm.eventListeners, elLi, "mousedown", Webfm.context.prototype.showContextMenu_listenerfn_mousedown(cp, obj));
			listener = Webfm.eventListenerAdd(Webfm.eventListeners, elLi, "mouseover", Webfm.context.prototype.showContextMenu_listenerfn_mouseover(cp));
			listener = Webfm.eventListenerAdd(Webfm.eventListeners, elLi, "mouseout", Webfm.context.prototype.showContextMenu_listenerfn_mouseout(cp));
			elUl.appendChild(elLi);
		}
	...
	listener = Webfm.eventListenerAdd(Webfm.eventListeners, document, "mousedown", function (e) {
																					cp.hideContextMenu(e);
																				});
...
Webfm.menuRename = function (obj) {
	...
	listener = Webfm.eventListenerAdd(Webfm.eventListeners, obj.tempInput, "blur", function (e) {
																					Webfm.renameActive = false;
																					obj.clickparent.replaceChild(obj.clickObj, obj.tempInput);
																					Webfm.stopEvent(e);  
																				});
	...
	listener = Webfm.eventListenerAdd(Webfm.eventListeners, obj.tempInput, "keydown", function (e) {
																						if (Webfm.enter(e) && Webfm.renameActive === true) { 
																							Webfm.renameActive = false; 
																							Webfm.menuSwapname(obj); 
																							this.blur(); 
																							Webfm.stopEvent(e);
																						}
																					});
...
Webfm.menuAttach = function (obj, ma_fid) {
	...
	for (i = 0; i < attach_arr.length; i = i + 1) {
		if (fid == attach_arr[i]) {
			break;
		}
	}
...
Webfm.perm.prototype.buildFormControls = function () {
	...
	listener = Webfm.eventListenerAdd(cp.eventListeners, submitButton, "click", function (e) {
																						if (cp.submitPerm()) {
	...
	listener = Webfm.eventListenerAdd(cp.eventListeners, resetButton, "click", function (e) {
																				cp.createForm(cp.data);
																				Webfm.stopEvent(e);
																			});
...
Webfm.search.prototype.createForm = function (source) {
	...
	listener = Webfm.eventListenerAdd(this.eventListeners, elInput, "keydown", function (e) {
																				if (Webfm.enter(e)) {
																					cp.submit(elInput.value);
																					elInput.blur();
																					Webfm.stopEvent(e);
																				}
																			});
	...
	listener = Webfm.eventListenerAdd(this.eventListeners, searchButton, "click", function (e) {
																					cp.submit(elInput.value);
																					Webfm.stopEvent(e);
																				});
...
Webfm.search.prototype.listenerfn_click_dir = function (cp) {
	var listenerfn =  function (e) {
								cp.selectDir(e);
								Webfm.stopEvent(e);
							};
...
Webfm.search.prototype.listenerfn_click_file = function (cp) {
	var listenerfn =  function (e) {
								cp.selectFile(e);
								Webfm.stopEvent(e);
							};
...
Webfm.search.prototype.listenerfn_mouseover = function (cp) {
	var listenerfn =  function (e) {
								cp.hoverDir(e, true);
							};
	return listenerfn;
...
Webfm.search.prototype.listenerfn_mouseout = function (cp) {
	var listenerfn =  function (e) {
								cp.hoverDir(e, false);
							};
	return listenerfn;
	...
	if (xmlhttp.status == 200) {
		...
		if (result.files.length) {
			...
			for (i = (result.files.length - 1); i >= 0; i = i - 1) {
				...
				listener = Webfm.eventListenerAdd(cp.eventListeners, elImg, "click", Webfm.search.prototype.listenerfn_click_dir(cp));
				listener = Webfm.eventListenerAdd(cp.eventListeners, elImg, "mouseover", Webfm.search.prototype.listenerfn_mouseover(cp));
				listener = Webfm.eventListenerAdd(cp.eventListeners, elImg, "mouseout", Webfm.search.prototype.listenerfn_mouseout(cp));
				...
				if (result.files[i].id) {
					...
					listener = Webfm.eventListenerAdd(cp.eventListeners, elA, "click", Webfm.search.prototype.listenerfn_click_file(cp));
				}
				else {
...
Webfm.pane = function (popupCont, title, paneId, content, paneWidth, contentHeight) {
	...
	cp = this;
	listener = Webfm.eventListenerAdd(Webfm.eventListeners, header, "mousedown", function (e) {
																					cp.dragStart(e, 1);
																				});
	...
	listener = Webfm.eventListenerAdd(Webfm.eventListeners, elResize, "mousedown", function (e) {
																						cp.dragStart(e, 0);
																					});
...
Webfm.draggable.prototype.drag = function (event) {
	...
	if (!Webfm.dragStart) {
		...
		if (!this.isAttach) {
			...
			if (dirListCont.length) {
				for (i = 0; i < dirListCont.length; i = i + 1) {
			...
			if (dirTreeCont) {
				...
				if (dirTreeCont.length) {
					...
					for (i = 0; i < dirTreeCont.length; i = i + 1) {
						// DragDrop element is folder icon
	...
	if (!this.isAttach) {
		...
		if (Webfm.dropContainers.length) {
			...
			for (i = 0; i < Webfm.dropContainers.length; i = i + 1) {
		...
		if (	(Webfm.attachContainer.x < pos.x) &&
			...
			for ( i = 1; i < attachRows.length; i = i + 1) {
...
Webfm.eventListenerRemove = function (event, listener_arr) {
	...
	for (i = 0; i < listener_arr.length; i = i + 1) {
